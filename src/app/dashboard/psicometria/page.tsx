...
type ResultadoGuardado = ResultadoPsicometria & {
  id: number;
  created_at: string;
};
...
export default function PsicometriaPage() {
  ...
  const [historial, setHistorial] = useState<ResultadoGuardado[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch("/api/psicometria", { cache: "no-store" });
        const json = await res.json();
        if (res.ok) setHistorial(json.resultados ?? []);
      } catch {}
      setCargando(false);
    })();
  }, [supabase, router]);
  ...
  async function enviar() {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error ?? "No se pudo registrar la prueba");
