  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        ...
        .eq("tipo", "satisfaccion_empleador")
        .limit(1);
      setYaRespondio((data?.length ?? 0) > 0);
    })();
  }, [supabase, router]);

  const likert = SATISFACCION_EMPLEADOR.filter((p) => p.tipo === "likert");
  const abiertas = SATISFACCION_EMPLEADOR.filter((p) => p.tipo === "texto");
  ...
  async function enviar() {
    ...
    try {
      const res = await fetch("/api/encuesta", {
        ...
        body: JSON.stringify({ tipo: "satisfaccion_empleador", respuestas }),
      });
      ...
      <div className="mx-auto max-w-2xl">
        <div className="card flex flex-col items-center gap-3 p-10 text-center">
          ...
          <p className="text-slate-500">
            Tu valoración sobre el desempeño de nuestros graduados nos ayuda a mejorar la
            formación y fortalece los procesos de acreditación.
          </p>
          <button
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <header>
        <span className="badge bg-teal-50 text-teal-700">
          <ClipboardCheck className="h-4 w-4" /> Encuesta a empleadores
        </span>
        <h1 className="mt-2 text-2xl font-bold text-slate-900">
          Satisfacción con nuestros graduados
        </h1>
        <p className="mt-1 text-slate-500">
          Evalúa el desempeño de los egresados de la universidad que trabajan en tu organización.
          Tus respuestas se procesan de forma agregada.
        </p>
      </header>
